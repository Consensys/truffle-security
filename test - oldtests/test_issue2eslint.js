...
describe('issues2Eslint', function() {
  describe('MythXIssues class', () => {
    ...
    it('should decode a source code location correctly', done => {
      ...
      assert.deepEqual(
        issuesObject.textSrcEntry2lineColumn(
          '30:2:0',
          issuesObject.lineBreakPositions[sourcePath]
        ),
    ...
    it('should decode a bytecode offset correctly', done => {
      const issuesObject = newIssueObject();
      assert.deepEqual(
        issuesObject.byteOffset2lineColumn(
          '100',
          issuesObject.lineBreakPositions[sourcePath]
        ),
    ...
    it('should convert MythX issue to Eslint style with sourceFormat: evm-byzantium-bytecode', () => {
      ...
      const issuesObject = newIssueObject();
      const res = issuesObject.issue2EsLint(
        remappedMythXOutput[0].issues[0],
        false,
    ...
    it('should call isIgnorable correctly', () => {
      const spyIsVariableDeclaration = sinon.spy(
        srcmap,
    ...
    it('should convert mythX report to Eslint issues', () => {
      ...
      const issuesObject = newIssueObject();
      const remappedMythXOutput = mythx.remapMythXOutput(mythXOutput);
      const result = remappedMythXOutput.map(output =>
        issuesObject.convertMythXReport2EsIssue(output, {}, true)
      );
    ...
    it('should not filter my issue if the project config is left empty', () => {
      ...
      const filteredIssues = issues.filter(issue =>
        issues2eslint.keepIssueInResults(issue, {})
      );
      ...
      const mythXOutput = [
        {
          ...
          meta: {
            ...
            warning: [],
            logs: [
              { level: 'info', msg: 'log message one' },
              { level: 'debug', msg: 'log message two' },
            ],
          },
        },
      ];
              ...
              locations: [
                {
                  ...
                  sourceFormat: 'text',
                  sourceList: [
                    'D:/Projects/mythx/test-contracts/contracts/Ens.sol',
                  ],
                },
              ],
              extra: { discoveryTime: 115950172, toolName: 'maru' },
              decodedLocations: [
                [{ line: 1, column: 0 }, { line: 1, column: 23 }],
              ],
            },
          ],
          sourceType: 'solidity-file',
          sourceFormat: 'text',
          sourceList: ['D:/Projects/mythx/test-contracts/contracts/Ens.sol'],
