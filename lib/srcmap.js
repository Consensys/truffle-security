...
module.exports = {
    ...
    */
    ...
    /**
     *  We use the heuristic that the first filemap entry of a srcmap will be
     *  the designated entry for the entire source collection. We believe that
     *  other files as a result say of "import" statements comes in the bytecode
     *  a little farther down. If this is not correct, there are other ways,
     *  such as using the AST, or seenIndices above.
     *
     *  @param {String} sourceMap     - solc-type sourceMap
     *  @return {Integer} first "file" entry found.
    */
    firstSrcMapIndex: function(sourceMap) {
        const srcArray = sourceMap.split(';');
        for (const src of srcArray) {
            const fields = src.split(':');
            if (fields.length >= 3) {
