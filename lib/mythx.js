    ...
    if (mythObject && mythObject.issues) {
        mythObject.issues.forEach(issue => {
            issue.locations.forEach(({ sourceMap }) => {
                ...
                // Potential hack fix for evm-byzantium-bytecode issue from post processor

                // if (typeof(mapped[sourceListIndex]) === 'undefined' && mapped[0].sourceFormat === 'evm-byzantium-bytecode') {
                //   sourceListIndex = 0;
                //   console.log(JSON.stringify(mapped[sourceListIndex].issues));
                // }
