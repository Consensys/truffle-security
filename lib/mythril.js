...
class Info {
    constructor(issues, buildObj) {
        ...
        const contractSource = buildObj.contractName;
...
exports.truffle2MythrilJSON = function(truffleJSON) {
    const clonedTruffleJSON = JSON.parse(JSON.stringify(truffleJSON));
    // Add/remap some fields because the Mythril Platform API doesn't
    // align with truffle's JSON

    clonedTruffleJSON.sourceList = [clonedTruffleJSON.contractName];
    clonedTruffleJSON.sources = {};
    clonedTruffleJSON.sources[clonedTruffleJSON.contractName] = clonedTruffleJSON.source;
    ...
    for (let field of [
        ...
        'source', 'sourcePath',
        'updatedAt']) {
        delete clonedTruffleJSON[field];
    }
    return clonedTruffleJSON;
};
