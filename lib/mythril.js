...
exports.truffle2MythrilJSON = function(truffleJSON) {
    ...
    // For now we need to *remove* fields that Mythril doesn't
    // understand. Hopefully this will change and we can remove the
    // code below. See:
    // https://github.com/ConsenSys/mythril-api/issues/58
    for (let field of [
...
exports.printIssues = function(issues) {
    for (let issue of issues) {
        for (let field of
            ['type', 'contract', 'function', 'code', 'address',
                'description']) {
            if (issue[field]) {
