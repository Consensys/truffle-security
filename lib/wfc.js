...
const Contracts = {
    ...
    compile: callbackify(async function(options) {
        ...
        const compilations = compilers.map(async (compiler) => {
            ...
            let [basenames, output] = await multiPromisify(compileFunc)(config);

            if (basenames && Object.keys(basenames).length > 0) {
                await this.writeContracts(basenames, config);
            }

            return { compiler, basenames, output };
        });
        ...
        const collect = async (compilations) => {
            ...
            for (let compilation of await Promise.all(compilations)) {
                let { compiler, output, basenames } = compilation;

		debugger
                result.outputs[compiler] = output;

                for (let [ name, abstraction ] of Object.entries(basenames)) {
                    result.basenames[name] = abstraction;
                }
    ...
    writeContracts: async function(contracts, options) {
        ...
        if (options.quiet != true && options.quietWrite != true) {
            const relPath = path.relative(options.working_directory, options.build_mythx_contracts);
            logger.log(`Writing artifacts to .${path.sep}${relPath}${OS.EOL}`);
        }

	for (const shortName of Object.keys(contracts)) {
	    const jsonData = JSON.stringify(contracts[shortName], null, 4);
